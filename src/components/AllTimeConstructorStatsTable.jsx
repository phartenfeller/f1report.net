...
export default function AllTimeConstructorStatsTable({ data }) {
    ...
    () => [
      {
        ...
        Cell: ({ row, value }) => (
           <div className="flex items-center space-x-2">
             ...
             <span>{value}</span>
             {row.original.legacy_color && <span className="text-xs text-zinc-500 italic ml-1">(Legacy)</span>}
           </div>
        )
      },
      {
  ...
  const [selected, setSelected] = useState([
      'name', 'points', 'races', 'podiums', 'wins', 'seconds', 'thirds', 'toptens', 'undertens'
  ]);
